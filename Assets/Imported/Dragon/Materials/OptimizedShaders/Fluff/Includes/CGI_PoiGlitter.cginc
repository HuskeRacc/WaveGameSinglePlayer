    ...
    void applyGlitter(inout float4 albedo, inout float3 glitterEmission)
    {
        ...
        // Scale
        float2 st = frac(poiMesh.uv[0] + float4(0,0,0,0).xy * _Time.x) * float(603.1);
        ...
        if(float(0))
        {
            glitterColor.rgb = hueShift(glitterColor.rgb, float(0.844) + _Time.x * float(10.86));
        }
