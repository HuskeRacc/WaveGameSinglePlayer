...
float3 calculateSpecular(in float4 albedo)
{
...

    {
        ...
        finalSpecular += calculateNewSpecular(specularMap.rgb, float(1), realisticAlbedo, float4(0.5911142,0.5911142,0.5911142,0.3921569), metallic, float(0.3) * specularMap.a, poiLight.dotNH, poiLight.dotLH, specularLightColor, attenuation);
    }
    ...
    if (float(1) == 4)
    {
...

        {
            ...
            finalSpecular += AnisotropicSpecular(float(0), float(0), float(0.3), float(0), float(1), float(1), float4(0.5911142,0.5911142,0.5911142,0.3921569), metallic, specularMap, specularLightColor, poiLight.direction, poiLight.halfDir, poiLight.nDotL, jitter, packedTangentMap, albedo);
            finalSpecular *= float4(0.5911142,0.5911142,0.5911142,0.3921569);
            finalSpecular *= attenuation;
        }
    #endif
        ...
        for (int index = 0; index < 4; index++)
        {
            ...
            {
                finalSpecular += calculateNewSpecular(specularMap.rgb, float(1), realisticAlbedo, float4(0.5911142,0.5911142,0.5911142,0.3921569), metallic, float(0.3) * specularMap.a, poiLight.vDotNH[index], poiLight.vDotLH[index], poiLight.vColor[index], poiLight.vAttenuationDotNL[index]);
            }
        }
    #endif
