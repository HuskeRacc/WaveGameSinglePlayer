...
void applyRimLighting(inout float4 albedo, inout float3 rimLightEmission)
{
    ...
    float rimWidth = lerp( - .05, 1, float(0.4));
    #ifdef POI_AUDIOLINK
    ...
    #if defined(PROP_RIMTEX) || !defined(OPTIMIZER_ENABLED)
        rimColor = POI2D_SAMPLER_PAN(_RimTex, _MainTex, poiMesh.uv[float(0)], float4(0,0,0,0)) * float4(0.02415762,0.3324516,0.6795426,1);
    #else
        rimColor = float4(0.02415762,0.3324516,0.6795426,1);
    #endif
    ...
    rim = 1 - smoothstep(min(float(0.05), rimWidth), rimWidth, viewDotNormal);
    rim *= float4(0.02415762,0.3324516,0.6795426,1).a * rimColor.a * rimMask;
    rimLightEmission = rim * lerp(albedo, rimColor, float(0.15)) * rimStrength;
    albedo.rgb = lerp(albedo.rgb, lerp(albedo.rgb, rimColor, float(0.15)) + lerp(albedo.rgb, rimColor, float(0.15)) * rimBrighten, rim);
}
#endif
